...
def _split_a_and_b(data, sent_ids, begin_idx, tot_len, extend_target=False):
    ...
    # print(len(data), len(sent_ids), begin_idx, tot_len, extend_target)
...
def _sample_mask(sp, seg, mask_alpha, mask_beta,
    ...
    while cur_len < seg_len:
        ...
        beg = cur_len + l_ctx
        while beg < seg_len and not _is_start_piece(sp.id_to_piece(seg[beg].item())):
            beg += 1
        if beg >= seg_len:
            break
        ...
        cnt_ngram = 1
        while end < seg_len:
            if _is_start_piece(sp.id_to_piece(seg[beg].item())):
                cnt_ngram += 1
                if cnt_ngram > n:
                    break
            end += 1
...
def _create_data(sp, input_paths, seq_len, reuse_len,
    ...
    for line in lines:
        ...
        cur_sent = sp.encode_as_ids(line)
