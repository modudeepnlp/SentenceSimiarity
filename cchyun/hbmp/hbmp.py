...
class SNLI(nn.Module):
    def __init__(self, config):
        ...
        self.layout1 = nn.Linear(config.d_hidden * 6 * 4, config.d_hidden * 6 * 2)
        self.dropout2 = nn.Dropout(p=config.dropout)
        self.layout2 = nn.Linear(config.d_hidden * 6 * 2, config.d_hidden * 6 * 1)
        self.layout3 = nn.Linear(config.d_hidden * 6 * 1, config.n_output)
...
class HBMP(nn.Module):
    ...
    def forward(self, inputs):
        ...
        h_0 = c_0 = Variable(inputs.data.new(self.config.n_layer * 2,
                                             batch_size,
                                             self.config.d_hidden).zero_())
