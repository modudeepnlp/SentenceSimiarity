...
def eval_epoch(model, data_loader, mode):
    ...
    with tqdm(total=len(data_loader), desc=f"{mode}") as pbar:
        for i, value in enumerate(data_loader):
            ...
            logit = model(sentences, segments)
...
def train_model(config, vocab, model, train_loader, valid_loader, test_loader):
    ...
    for epoch in trange(config.n_epoch, desc="Epoch"):
        score_loss = train_epoch(epoch, model, loss_fn, optimizer, train_loader)
        ...
        if best_test is None or best_test < socre_test:
            model.save("bert_final.pth")
            best_epoch, best_loss, best_val, best_test = epoch, score_loss, score_val, socre_test
            print(">>>>>>>", "model saved at", "bert_final.pth", best_epoch, best_loss, best_val, best_test)
...
def main():
    ...
    if os.path.isfile("bert_final.pth"):
        model.load("bert_final.pth")
        print(">>>> load state dict from: ", "bert_final.pth")
